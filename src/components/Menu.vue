<template>
  <sui-menu attached="top">
    <sui-dropdown item icon="bars" simple class="mobile only">
      <sui-dropdown-menu>
        <template v-for="item in menu">
          <sui-dropdown-header v-if="!item.to" :key="item.label">{{item.label}}</sui-dropdown-header>
          <sui-dropdown-divider v-if="!item.label" :key="item.label"/>
          <router-link active-class="active" class="item" :to="item.to" :key="item.label" v-if="item.to">{{item.label}}</router-link>
        </template>
      </sui-dropdown-menu>
    </sui-dropdown>
    ...
    <router-link v-for="item in menuItems" active-class="active" class="item mobile hidden" :to="item.to" :key='item.to'>{{item.label}}</router-link>
  </sui-menu>
</template>
